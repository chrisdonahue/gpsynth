...
SquareFreqOscNode::SquareFreqOscNode(GPMutatableParam* partial, GPMutatableParam* phase, GPNode* freq) {
    ...
    symbol = "squarefreqosc";
}

SquareFreqOscNode::~SquareFreqOscNode() {
}
...
SquareFreqOscNode* SquareFreqOscNode::getCopy() {
    return new SquareFreqOscNode(mutatableParams[0]->getCopy(), mutatableParams[1]->getCopy(), descendants[0] == NULL ? NULL : descendants[0]->getCopy());
}

void SquareFreqOscNode::setRenderInfo(float sr, unsigned blockSize, unsigned maxFrameNumber, float maxTime) {
	WaveTableFreqNode::setRenderInfo(sr, blockSize, maxFrameNumber, maxTime);
}

void SquareFreqOscNode::updateMutatedParams() {
...
void SquareFreqOscNode::makeAddAllWaveTables(double sampleRate, unsigned overSamp, unsigned constantRatioLimit, double baseFrequency, double topFrequency) {
...
void SquareFreqOscNode::defineHarmonics(int len, int numHarmonics, double* ar, double* ai) {
