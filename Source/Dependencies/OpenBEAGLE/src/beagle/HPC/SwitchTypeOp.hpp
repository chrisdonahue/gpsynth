...
public:
	...
	*/
	inline const Operator::Bag& getSet(const std::string& inType) const
	{
		...
		return lIter->second;
		Beagle_StackTraceEndM("const Operator::Bag& SwitchTypeOp::getSupervisorSet() const");
	}
	...
	void insertOp(const std::string& inType, Operator::Handle inOperator)
	{
		...
		Beagle_StackTraceEndM("Operator::Bag& SwitchTypeOp::insertEvaluatorOp(Operator::Handle inOperator)");
	}
