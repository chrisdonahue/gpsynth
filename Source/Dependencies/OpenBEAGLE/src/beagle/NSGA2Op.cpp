...
public:
	...
	inline bool operator()(const CrowdingPair& inLeftPair, const CrowdingPair& inRightPair) const
	{
		...
		Beagle_StackTraceEndM("bool IsLessCrowdingPairPredicate::operator()(const CrowdingPair& inLeftPair, const CrowdingPair& inRightPair) const");
	}
private:
...
void NSGA2Op::applyAsReplacementStrategy(Deme& ioDeme, Context& ioContext)
{
	...
	Beagle_LogVerboseM(
	    ioContext.getSystem().getLogger(),
	    "replacement-strategy", "Beagle::NSGA2Op",
	    std::string("Generating ")+uint2str(lLambda)+
	    std::string(" offsprings using breeder tree")
	);
	...
	Beagle_LogVerboseM(
	    ioContext.getSystem().getLogger(),
	    "replacement-strategy", "Beagle::NSGA2Op",
	    std::string("Computing crowding distance on the ")+uint2ordinal(lParetoFronts.size())+
	    std::string(" Pareto front, which is made of ")+uint2str(lParetoFronts.back().size())+
	    " individuals"
	);
...
void NSGA2Op::registerParams(System& ioSystem)
{
	...
	Beagle_StackTraceEndM("void NSGA2Op::registerParams(System&)");
}
...
void NSGA2Op::init(System& ioSystem)
{
	...
	Beagle_StackTraceEndM("void NSGA2Op::init(System&)");
}
...
void NSGA2Op::readWithSystem(PACC::XML::ConstIterator inIter, System& ioSystem)
{
	...
	Beagle_StackTraceEndM("void NSGA2Op::readWithSystem(PACC::XML::ConstIterator, System&)");
}
