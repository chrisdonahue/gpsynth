...
void AlgoNSGA2::configure(Evolver& ioEvolver, System& ioSystem)
{
	...
	std::string lNSGA2OpName = "NSGA2Op";
	NSGA2Op::Alloc::Handle lNSGA2OpAlloc =
	    castHandleT<NSGA2Op::Alloc>(lFactory.getAllocator(lNSGA2OpName));
	...
	NSGA2Op::Handle lNSGA2Op = castHandleT<NSGA2Op>(lNSGA2OpAlloc->allocate());
	lNSGA2Op->setName(lNSGA2OpName);
	ioEvolver.getMainLoopSet().push_back(lNSGA2Op);

	// Set crossover branch
	BreederNode::Handle lCxBranchNode = new BreederNode;
	lNSGA2Op->setRootNode(lCxBranchNode);
	...
	Beagle_StackTraceEndM("void AlgoNSGA2::configure(Evolver&,System&)");
}
