...
The fitness function I am using is a variation of the fitness function that Garcia used for his system. The target wave form is partitioned into groups of $n$ non-overlapping frames. These groups are analyzed using a discrete Fourier transform which produces spectral data at $n/2$ bins equally spaced between $0$ and $sample rate/2$ Hertz. Each group $i$ of $n/2$ bins is analyzed to find the minimum magnitude bin $Min_{i}$, maximum magnitude bin $Max_{i}$, and average magnitude $Avg_{i}$. Each bin $j$ is assigned a penalty for a candidate overshooting that particular bin and a penalty for a candidate undershooting that particular bin in the following way.

$|T_{ij}|$ is the magnitude of bin $j$ from group $i$.
