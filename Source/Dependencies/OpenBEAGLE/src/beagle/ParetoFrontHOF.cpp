...
const std::string& ParetoFrontHOF::getType() const
{
	...
	return lType;
	Beagle_StackTraceEndM("const std::string& ParetoFrontHOF::getType() const");
}
...
bool ParetoFrontHOF::updateWithDeme(unsigned int inSizeHOF, const Deme& inDeme, Context& ioContext)
{
		...
		for(unsigned int j=0; j<mEntries.size(); ++j) {
			...
			if(lIndivFitness->isDominated(*lEntryFitness)) {
				...
				Beagle_LogDebugM(
				    ...
				    "hall-of-fame",
				    "Beagle::ParetoFrontHOF",
				    "Individual removed from Pareto front"
				);
				Beagle_LogObjectDebugM(
				    ioContext.getSystem().getLogger(),
				    "hall-of-fame",
				    "Beagle::ParetoFrontHOF",
				    *mEntries[j].mIndividual
				);
		...
		if(lIsDominated==false) {
			...
			if(lIsIdentical == false) {
				...
				Beagle_LogDebugM(
				    ...
				    "hall-of-fame",
				    "Beagle::ParetoFrontHOF",
				    "Individual added to Pareto front"
				);
				Beagle_LogObjectDebugM(
				    ioContext.getSystem().getLogger(),
				    "hall-of-fame",
	...
	if(lIndividualsAdded != 0) {
		Beagle_LogVerboseM(
		    ...
		    string("There are ")+uint2str(lIndividualsAdded)+
	...
	if(lIndividualsRemoved != 0) {
		Beagle_LogVerboseM(
		    ioContext.getSystem().getLogger(),
		    "hall-of-fame", "Beagle::ParetoFrontHOF",
		    string("There are ")+uint2str(lIndividualsRemoved)+
	...
	Beagle_StackTraceEndM("bool ParetoFrontHOF::updateWithDeme(unsigned int,const Deme&,Context&)");
}
        ...
        Context& ioContext)
{
	...
	Beagle_StackTraceEndM("bool ParetoFrontHOF::updateWithIndividual(unsigned int,const Individual&,Context&)");
}
