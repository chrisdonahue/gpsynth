...
The fitness function used for these tests was designed to best compare the five parameters listed in \ref{INTRO}. The target samples are partitioned into groups of $n$ with $o$ overlap. These groups are analyzed using a discrete Fourier transform which produces spectral data at $n/2$ bins equally spaced between $0$ and $sample rate/2$ Hertz. The exponential moving average and highest deviation above and below the exponential moving average is calculated for each group $i$ of $n/2$.

Each group $i$ has the following values:
\begin{enumerate}
$MAXABOVE_{i}$: $\max{MAG_{i, j} - EMAC_{i, j}}$
$MAXBELOW_{i}$: $\max{EMAC_{i, j} - MAG_{i, j}}$
\end{enumerate}

Each bin $j$ of group $i$ now has the following values:
\begin{enumerate}
$EMAC_{i, j}$: Exponential moving average curve at group $i$ bin $j$
$MAG_{i, j}$: Magnitude of the frequency component at group $i$ bin $j$
$PHS_{i, j}$: Phase of the frequency component at group $i$ bin $j$
\end{enumerate}
...
\begin{equation}
if MAG_{i, j} <= EMAC_{i, j}:
	UNDER_{i, j} =  ({\abs{MAG_{i, j} - EMAC_{i, j}}}/{MIN{I}} * good) + base;
	OVER_{i, j} =  ({\abs{MAG_{i, j} - EMAC_{i, j}}}/{MIN{I}} * bad) + base
if MAG_{i, j} > EMAC_{i, j}:
	UNDER_{i, j} =  ({\abs{MAG_{i, j} - EMAC_{i, j}}}/{MIN{I}} * bad) + base
	OVER_{i, j} =  ({\abs{MAG_{i, j} - EMAC_{i, j}}}/{MIN{I}} * good) + base
\end{equation}

Every candidate is assigned a fitness score in the following way:
