...
/*!
 *  \file   beagle/GA/src/AdaptOneFifthRuleFltVecOp.cpp
 *  \brief  Source code of class GA::AdaptOneFifthRuleFltVecOp.
 ...
 */
GA::AdaptOneFifthRuleFltVecOp::AdaptOneFifthRuleFltVecOp(std::string inSigmaName, std::string inName) :
		BreederOp(inName),
 ...
 */
double GA::AdaptOneFifthRuleFltVecOp::getBreedingProba(BreederNode::Handle inChild)
{
	...
	Beagle_StackTraceEndM("double GA::AdaptOneFifthRuleFltVecOp::getBreedingProba(BreederNode::Handle)");
}


/*!
 *  \brief Register parameters for the real-valued GA Gaussian mutation operator.
 *  \param ioSystem System of the evolution.
 */
void GA::AdaptOneFifthRuleFltVecOp::registerParams(System& ioSystem)
{
	Beagle_StackTraceBeginM();
	BreederOp::registerParams(ioSystem);
	{
		...
		mAdaptationPeriod = castHandleT<UInt>(
		                        ioSystem.getRegister().insertEntry("ga.adapt.period", new UInt(10), lDescription));
	}
	{
		std::ostringstream lOSS;
		...
		mSuccessCount = castHandleT<UInt>(
		                    ioSystem.getRegister().insertEntry("ga.adapt.success", new UInt(0), lDescription));
	}
	{
		std::ostringstream lOSS;
		...
		mMutationsCount = castHandleT<UInt>(
		                      ioSystem.getRegister().insertEntry("ga.adapt.count", new UInt(0), lDescription));
	}
	{
		std::ostringstream lOSS;
		...
		mSigmaAdaptFactor = castHandleT<Double>(
		                        ioSystem.getRegister().insertEntry("ga.adapt.alpha", new Double(0.85), lDescription));
	}
	{
		std::ostringstream lOSS;
	...
	Beagle_StackTraceEndM("void GA::AdaptOneFifthRuleFltVecOp::registerParams(System&)");
}
 ...
 */
Individual::Handle GA::AdaptOneFifthRuleFltVecOp::breed(Individual::Bag& inBreedingPool,
        ...
        Context& ioContext)
{
	...
	if(lFitChild != NULL) {
		...
		if(lSuccessful) {
			...
			Beagle_LogVerboseM(
			    ...
			    "adaptation", "Beagle::GA::AdaptOneFifthRuleFltVecOp",
			    "Successful mutation"
			);
		} else {
			Beagle_LogVerboseM(
	...
	return lIndiv;
	Beagle_StackTraceEndM("Individual::Handle GA::AdaptOneFifthRuleFltVecOp::breed(Individual::Bag& inBreedingPool,BreederNode::Handle inChild,Context& ioContext)");
}
 ...
 */
void GA::AdaptOneFifthRuleFltVecOp::operate(Deme& ioDeme, Context& ioContext)
{
	Beagle_StackTraceBeginM();
	throw Beagle_UndefinedMethodInternalExceptionM("operate", "GA::AdaptOneFifthRuleFltVecOp", getName());
	Beagle_StackTraceEndM("void GA::AdaptOneFifthRuleFltVecOp::operate(Deme& ioDeme, Context& ioContext)");
}
 ...
 */
void GA::AdaptOneFifthRuleFltVecOp::readWithSystem(PACC::XML::ConstIterator inIter, System& ioSystem)
{
	Beagle_StackTraceBeginM();
	...
	Beagle_StackTraceEndM("void GA::AdaptOneFifthRuleFltVecOp::readWithSystem(PACC::XML::ConstIterator, System&)");
}
 ...
 */
void GA::AdaptOneFifthRuleFltVecOp::writeContent(PACC::XML::Streamer& ioStreamer, bool inIndent) const
{
	Beagle_StackTraceBeginM();
	Beagle::BreederOp::writeContent(ioStreamer, inIndent);
	ioStreamer.insertAttribute("mutgausssigma", mMutateGaussSigmaName);
	Beagle_StackTraceEndM("void GA::AdaptOneFifthRuleFltVecOp::writeContent(PACC::XML::Streamer& ioStreamer, bool inIndent) const");
}
