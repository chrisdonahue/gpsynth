...
/*!
 *  \file   beagle/src/MigrationMapOp.cpp
...
void MigrationMapOp::migrate(Deme& ioDeme, Context& ioContext)
{
	...
	mSelectionOp->selectNIndividuals(lNbEmigrants, ioDeme, ioContext, lEmigrantIndices);
	...
	if(mReplacementOp == NULL) lReplacedIndices = lEmigrantIndices;
		...
		ioContext.setSelectionIndex(0);
		mReplacementOp->selectNIndividuals(lNbEmigrants, ioDeme, ioContext, lEmigrantIndices);
...
{
	Beagle_StackTraceBeginM();
	Beagle::MigrationOp::init(ioSystem);
